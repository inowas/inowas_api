...
class LoadScenario_3 implements FixtureInterface, ContainerAwareInterface
{
    ...
    public function load(ObjectManager $entityManager)
    {
        ...
        $project->setDescription('This project contains real data of Hanoi, provided by Aybulat (Python-Hero) Fatkhutdinov.');
        $entityManager->persist($project);
        $entityManager->flush();

        // Create new geological layers
        $layer = GeologicalLayerFactory::setOwnerProjectNameAndPublic($user, $project, 'SC3_L1', $public);
        $entityManager->persist($layer);
        $layer = GeologicalLayerFactory::setOwnerProjectNameAndPublic($user, $project, 'SC3_L2', $public);
        $entityManager->persist($layer);
        $layer = GeologicalLayerFactory::setOwnerProjectNameAndPublic($user, $project, 'SC3_L3', $public);
        $entityManager->persist($layer);
        $entityManager->flush();
        ...
        // Add new Area
        $area = AreaFactory::setOwnerProjectNameTypeAndPublic($user, $project, 'SC3_A1', $areaType, $public);
        ...
        // Add new boundaries
        $boundary = BoundaryFactory::setOwnerProjectNameAndPublic($user, $project, 'SC3_B1', $public);
        ...
        $boundary = BoundaryFactory::setOwnerProjectNameAndPublic($user, $project, 'SC3_B2', $public);
